...
### 2026-05-24 — PR-BETA-2 i18n perf Phase B + landing FR/EN drift (THI-266)
...
- **THI-266 test(i18n)** — 3 nouveaux Vitest specs `LocaleSwitcher — THI-266 Phase B no-refresh contract` qui verrouillent le contrat : `router.replace` appelé avec `(pathname, { locale })`, `router.refresh` **jamais** appelé, ordre des side-effects `setLocaleAction` puis `replace` garanti. Refactor du mock `@/i18n/navigation` via `vi.hoisted()` pour exposer les mock fns à la fois au factory `vi.mock` et aux assertions.
- **THI-266 e2e(i18n)** — Les 3 scenarios `test.fixme()` de `e2e/i18n/locale-switcher.spec.ts` (créés en PR #177 Phase A) sont **unfixme'd** : rapid switches FR↔EN settle on last selection, locale survives cross-page navigation, parité i18n sur les routes principales. Helper `switchTo()` resserre son timeout 15 s → 5 s (résistant au HMR jitter de `npm run dev` sans masquer une régression). Header JSDoc + commentaires inline mis à jour pour refléter Phase B done.
...
- **Hors scope (volontairement reporté)** — Extraction de `cookies()` hors de `[locale]/layout.tsx` + audit du middleware matcher (audit perf THI-243 RC #2 / #4) : reporté à **PR-BETA-2bis** après mesure perf chiffrée. Diagnostic affiné pendant Phase 0 du présent travail : la cause perçue de la lenteur n'est pas le statut `ƒ Dynamic` des routes (qui reste inchangé par cette PR) mais bien le `router.refresh()` redondant + le cold-render Supabase dans `src/i18n/request.ts` (deux round-trips DB par request quand `NEXT_LOCALE` absent). Le drawer-stay-open et la propagation `< 500 ms` sont résolus structurellement par le retrait du `refresh`, sans toucher au layout racine ni au middleware. Si la mesure prod post-merge confirme le gain attendu, PR-BETA-2bis vise uniquement l'optimisation `request.ts` (cookie-first path, skip DB lookup). Détails arbitrage : `docs/prs/PR-BETA-2-i18n-perf-phase-b-report.md` §3.
