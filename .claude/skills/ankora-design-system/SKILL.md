...
- **Barre de rythme** (`components/dashboard/PaceBar.tsx`) — dénominateur « Budget du mois », remplissage « Dépensé ce mois », repère à `joursEcoules / joursDuMois`. Elle ne demande **aucune saisie** : c'est ce qui la rend supérieure à une enveloppe. Verdict en mots, jamais en flèche.
