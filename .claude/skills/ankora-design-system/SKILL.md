...
- **Courbe du mois** — trois fichiers, et le cockpit rend le TROISIÈME. `src/components/dashboard/month-curve-geometry.ts` (géométrie pure, se prouve sans DOM) · `src/components/dashboard/MonthCurve.tsx` (couleur, libellés, projection, nom accessible) · **`src/components/dashboard/MonthCurveLive.tsx`, la seule surface montée par le cockpit** (`SituationDuMoisHero.tsx:295`) : elle branche la courbe sur le magasin optimiste. Cumul des dépenses jour par jour, repère de rythme à `joursEcoules / joursDuMois`, projection de fin de mois en pointillé. Elle ne demande **aucune saisie** : c'est ce qui la rend supérieure à une enveloppe. Verdict en mots, jamais en flèche. **Un seul niveau d'alarme par écran** : dépasser le budget prend `--color-danger`, aller plus vite que le mois prend `--color-warning` — c'est un fait, pas une alarme. Elle ne dit jamais « tu dépenses trop » (R-06). **La courbe ne recalcule JAMAIS le total depuis sa propre série : elle dessine le nombre qu'on lui tend** (règle 10). Dans le cockpit, `MonthCurveLive` lui tend la figure **optimiste** pendant la fenêtre d'attente, et la vérité serveur le reste du temps — le hero et la courbe bougent donc du même nombre, par construction. Attention, coût assumé et documenté dans `MonthCurveLive.tsx:37-48` : pendant cette fenêtre le tracé bouge alors que l'`aria-label` énonce encore les montants d'avant. _(Remplace `PaceBar.tsx`, supprimé en #451 — retiré de cette liste le 25 août 2026, même motif que « Pricing » au §10 : une prescription qui nomme un fichier absent envoie écrire du code contre un patron introuvable. La barre plafonnait son remplissage, donc un dépassement de 300 % ressemblait à 1 % ; l'échelle de la courbe s'ouvre.)_
