...
- **Dashboard user (cockpit)** — `src/app/[locale]/app/page.tsx` orchestre les surfaces, qui rendent via `src/components/dashboard/*` (SituationDuMoisHero + AllocationBar, ProvisionHealthGaugeCard, ProchainesFacturesCard) + `src/components/features/AccountCard.tsx` (compte épargne · trois lectures). Hero « Situation du mois » (waterfall revenus→charges→provisions→reste disponible→épargne, THI-327 ; subsume les anciennes cartes Effort + Capacité), Santé Provisions (THI-190), Prochaines factures (THI-192), Assistant Virements (ADR-012).
