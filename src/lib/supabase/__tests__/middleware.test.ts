...
describe('updateSession — an outage must not log anybody out', () => {
  const outages = [
    ['a transport failure', { name: 'AuthRetryableFetchError', message: 'Failed to fetch', status: 0, __isAuthError: true }],
    ['a 503 from the auth server', { name: 'AuthApiError', message: 'unavailable', status: 503, __isAuthError: true }],
    ['rate limiting', { name: 'AuthApiError', message: 'too many requests', status: 429, __isAuthError: true }],
  ] as const;
