...
- **Amendé par** :
  - [ADR-040](ADR-040-ordre-execution-du-journal.md) (2026-08-10) — **lire ADR-040 avant d'exécuter quoi que ce soit d'ici.** Il inverse le §Découpage d'exécution (D0 passe en dernier), déplace `commitments.paid_from` de D0 vers D3, corrige deux faits de schéma cités de travers, retire la surévaluation du risque de D0, et ajoute trois décisions (D10 reliquat, D11 cohérence/exhaustivité, D12 invariants). **Aucune décision de fond de cet ADR n'est annulée.**
  - [ADR-041](ADR-041-provisionner-nest-pas-payer.md) (2026-08-10) — **D3 était ambigu et D6 en dépend.** `charges.paid_from` désigne le compte qui _provisionne_ ; `paid_from_account_type` désigne le compte qui _paie_. Cet ADR les traitait implicitement comme un seul. ADR-041 les sépare, ajoute à `accounts` la capacité de régler directement (qui n'est ni universelle ni déductible de la banque — sources datées) et fait porter à D1 l'écriture des **deux** mouvements quand l'enveloppe ne règle pas elle-même. Aucune décision de fond n'est annulée ; D3 gagne le sens qui lui manquait.
