...
- **Observation systémique a11y** (hors scope, à tracer) : `--color-success` #059669 = 3.77:1 sur card claire et `--color-danger` #dc2626 = 3.59:1 sur card sombre sont **sous 4.5:1** → tout futur petit texte coloré échouera AA. Les usages existants (hero text-xl, épargne text-lg = grand texte, seuil 3:1) restent conformes. Un override dark-mode des tokens (`success→#34d399`, `danger→#f87171`) réglerait le système mais a un blast radius app-wide → PR a11y dédiée recommandée, pas ici.
