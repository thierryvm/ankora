...
Dernière mise à jour : 6 mai 2026 — **Voie D Dashboard Cockpit en cours**. PR-D1 Foundations (#94) ✅ + PR-D2 Header + 3 cards comptes (#96) ✅ mergées. Sprint Mobile Recovery Ankora ✅ clos (PR-QA-1a/1b/1c-0/1c-1, mockups iPhone 14+ Safari WebKit validés). Prochaine étape : **PR-D3 Effort Lissé + Capacité Réelle** (KPI hero différenciateur Ankora vs concurrents) démarrage **lundi 12 mai 2026**, post-Session Claude Design #3 (week-end 10-11 mai, fenêtre Opus 4.7 @thierry). **Décision Option B tranchée 6 mai par @cowork** (carte blanche @thierry) : alpha @thierry + 2-3 proches 4-6 sem, **PAS de release publique 10 mai**, v1.0 publique mi-juin. Cf. [ADR-005](./adr/ADR-005-pr3a-anticipated-design-system.md) pour re-séquencement PR-3 + handoff Obsidian `Athenaeum/10_Projects/ankora/cowork-handoffs/2026-05-06-1230-anchor-ankora-cd3-launch.md`.
...
| 11  | **Voie D — Dashboard Cockpit complet (PR-D1 → PR-D8)**     | 🚧 EN COURS. PR-D1 (#94) ✅ + PR-D2 (#96) ✅ mergées. 5 ADRs (008-012) ✅ `Accepted` (validés 2026-05-03). Reste PR-D3 → PR-D8 (démarrage 12 mai post-mockups CD#3).                       | —             | Refactor complet vers la vraie vision IronBudget : Effort Lissé, Capacité Réelle, Assistant Virements, Plan rattrapage, Live Quotidien, Simulateur d'Action |
...
| PR-D3 | **Bloc 1 radar : Effort Lissé + Capacité Réelle**                         | PR-D2 mergée                                            | 2 cards radar avec breakdown + glow + message contextuel. Tests Vitest sur cas limites (revenus = 0, capacité < 0).                                                                                                                                                       | 1-2 jours  |
| PR-D4 | **Liste charges "À payer en {mois}" + toggle paye**                       | PR-D3 mergée                                            | Refonte form `/charges` (multi-mois + catégorie + jour). Composant `ChargeList` + sub-sections par compte source. Toggle paye (charge_payments). Tri date / ordre custom + drag&drop. Edit inline.                                                                        | 3 jours    |
